...
export function VendorsScreen() {
  ...
  const selected = active
    ? (vendors.find((v) => v.name === active) ?? null)
    : null;
    ...
    <div className="inbox-content">
      ...
      <div className="border border-hairline bg-surface">
        <table className="table">
          <thead>
            <tr>
              <SortHead k="name" sort={sort} set={setSort}>
                Vendor
              </SortHead>
              <SortHead k="invoices" sort={sort} set={setSort} right>
                Invoices
              </SortHead>
              <SortHead k="totalSpent" sort={sort} set={setSort} right>
                Total spent
              </SortHead>
              <SortHead k="avgInvoice" sort={sort} set={setSort} right>
                Avg
              </SortHead>
              <SortHead k="lastSeen" sort={sort} set={setSort}>
                Last seen
              </SortHead>
              <th>Trend</th>
              <SortHead k="confidence" sort={sort} set={setSort} right>
                Confidence
              </SortHead>
            </tr>
          </thead>
          <tbody>
            {isLoading && (
              <SkeletonRows rows={6} cols={["38%", 56, 80, 60, 90, 84, 70]} />
            )}
            {!isLoading && sorted.length === 0 && (
              <tr>
                ...
                >
                  No vendors yet. Upload an invoice in the Inbox to start
                ...
                <td className="font-medium">
                  ...
                  {v.anomalies > 0 && (
                    <span className="mono ml-2 border border-[#ebd0a8] bg-triage-needs-review-tint px-1.5 py-px text-[12px] text-triage-needs-review">
                      {v.anomalies}{" "}
                      {v.anomalies === 1 ? "anomaly" : "anomalies"}
                    </span>
                  )}
                </td>
...
function ScreenHeader({
  ...
  return (
    <div className="mb-5">
      ...
      <div className="max-w-[70ch] text-sm leading-[1.5] text-ink-60">
        {sub}
      </div>
    </div>
  );
  ...
  return (
    <div className={`px-5 py-4 ${last ? "" : "border-r border-hairline"}`}>
      ...
      <div className="mt-1.5 flex items-baseline gap-2">
        ...
        {suffix && (
          <span className="font-mono text-xs text-ink-60">{suffix}</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="fixed bottom-0 right-0 top-[60px] z-30 w-[440px] overflow-y-auto border-l border-hairline bg-surface shadow-[0_8px_32px_rgba(0,0,0,0.12)]">
      ...
      <div className="grid grid-cols-2 gap-2.5 px-[18px] py-4">
        ...
        <KV
          label="Total spent"
          value={
            vendor.totalSpent > 0 ? formatMoneyFull(vendor.totalSpent) : "–"
          }
        />
        <KV
          label="Avg invoice"
          value={
            vendor.avgInvoice > 0 ? formatMoneyFull(vendor.avgInvoice) : "–"
          }
        />
